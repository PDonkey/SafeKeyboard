...
public class SafeKeyboard {
    ...
    private void initKeyboard() {
        ...
        // TODO: 2019/1/8  以下重新赋值keycode, 例如点击q键这里把他赋值给了a，输入框里就是a (以下3个for循环删除就是正常的键盘)
        //赋值英文字母键盘keycode
        for (int i = 0; i < MainActivity.getLetterChars().size(); i++) {
            // 19
            if (i >= 19) {
                keyboardLetter.getKeys().get(i + 1).codes[0] = MainActivity.stringToAscii(MainActivity.getLetterChars().get(i));
            } else {
                keyboardLetter.getKeys().get(i).codes[0] = MainActivity.stringToAscii(MainActivity.getLetterChars().get(i));
            }
        }
        //赋值数字键盘keycode
        for (int i = 0; i < MainActivity.getNumChars().size(); i++) {
            if (i == 9) {
                keyboardNumber.getKeys().get(i + 1).codes[0] = MainActivity.stringToAscii(MainActivity.getNumChars().get(i));
            } else {
                keyboardNumber.getKeys().get(i).codes[0] = MainActivity.stringToAscii(MainActivity.getNumChars().get(i));

            }
        }
        //赋值符号键盘keycode
        for (int i = 0; i < MainActivity.getSymbolChars().size(); i++) {
            // 29
            if (i >= 28) {
                keyboardSymbol.getKeys().get(i + 2).codes[0] = MainActivity.stringToAscii(MainActivity.getSymbolChars().get(i));
            } else {
                keyboardSymbol.getKeys().get(i).codes[0] = MainActivity.stringToAscii(MainActivity.getSymbolChars().get(i));
            }
        }
        ...
        public void onKey(int primaryCode, int[] keyCodes) {
            try {
                ...
                if (primaryCode == Keyboard.KEYCODE_CANCEL) {
                    ...
                    String sss = Character.toString((char) primaryCode);
